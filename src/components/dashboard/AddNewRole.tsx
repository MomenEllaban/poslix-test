...
const AddNewRole = (probs: any) => {
  ...
  async function insertUpdateUsers() {
    const { success, newdata, msg } = await apiInsert({
      type: 'addUpdatebusinessRoles',
      data: { data: formObj, pages: pages },
    });
    ...
    if (formObj.isNew) probs.stuffs.push(newdata);
    else {
