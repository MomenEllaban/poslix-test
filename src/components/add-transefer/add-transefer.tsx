      ...
      <div className={styles.add_transefer_wrapper}>
        ...
        <div className="d-flex gap-3 mb-3 flex-wrap">
          ...
          <TextField
            ...
            sx={{ width: '30%' }}
            id="ref no"
            label={t('transfers.Referance_Number')}
            placeholder={t('transfers.Reference_Number')}
            variant="outlined"
            size="small"
          ...
          <Autocomplete
            ...
            options={
              fromProducts.filter((fromProduct) => {
                return (
                  toProducts.some(
                    (toProduct) => fromProduct.name?.trim() === toProduct.name?.trim()
                  ) &&
                  !selectedProducts.some(
                    (selectedProduct) => fromProduct.name?.trim() === selectedProduct.name?.trim()
                  )
                );
              }) || []
            }
            ...
            renderInput={(params) => (
              <TextField
                ...
                label={t('transfers.Select_Product')}
              />
            )}
          />

          {/* status select start */}
          <FormControl size="small" sx={{ width: '30%', marginBottom: '8px' }}>
            <InputLabel id="select-product-label">
              {t('transfers.Select_Product_Status')}
            </InputLabel>
            <Select
              size="small"
              ...
              <MenuItem value="received">{t('transfers.Received')}</MenuItem>

              <MenuItem value="partially_received">{t('transfers.Partially_Received')}</MenuItem>
              <MenuItem value="processing">{t('transfers.Processing')}</MenuItem>
              <MenuItem value="cancelled">{t('transfers.Cancelled')}</MenuItem>
            </Select>
          </FormControl>
          {/*  */}
          ...
          {selectedProducts?.map((el, i) => {
            return (
              <Item
                ...
                fromLocation={from}
                key={el.id}
                reduceProductQty={reduceProductQty}
