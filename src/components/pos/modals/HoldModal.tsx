...
const HoldModal = (props: any) => {
  const { openDialog, isShowModal, holdObj, shopId } = props;
  ...
  useEffect(() => {
    if (isShowModal) {
      ...
      if (holdItemsFromStorage) setHoldItems(JSON.parse(holdItemsFromStorage));
      holdObj.orders.map((p: any, i: number) =>
        orderIds.push({
          type: p.type,
          ...
          data: '',
          qty: 0,
          tailoring: holdObj.quantity[i].tailoring,
          tailoringCutsom: holdObj.quantity[i].tailoringCutsom,
        })
      );
    } else setOrderIds([]);
  }, [isShowModal]);
  ...
  return (
    <>
      <Dialog open={isShowModal} sx={style} className="poslix-modal">
        <DialogTitle className="poslix-modal text-primary">Hold Orders</DialogTitle>
        <DialogContent
          className="poslix-modal-content"
          style={{ minWidth: '400px', minHeight: '180px' }}>
                  ...
                  onClick={() => {
                    if (holdObj.orders.length > 0) {
                      if (newItem.name.length > 0) {
                        ...
                        setClear(random);
                        openDialog(false);
                      } else Toastify('error', 'Enter Hold Name First!');
                    } else Toastify('error', 'There Is Nothing For Hold!');
                  }}>
                  Save
                </button>
              </div>
