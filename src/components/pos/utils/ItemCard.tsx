    ...
    <>
      <div
        ...
        <div className="img-container">
          <img
            src={items.image?.length > 10 ? items.image : '/images/pos/placeholder.png'}
            alt={items.name}
            className="img-fluid"
          />
        </div>
      ...
      <div className="item-price">
        {!items.type?.includes('variable')
          ? Number(items.price).toFixed(3) + locationSettings?.currency_code
          : '--'}
      </div>
      {!items.is_service &&
        !items.type?.includes('variable') &&
        !items.type?.includes('package') &&
        items.total_qty == 0 &&
        !items.sell_over_stock && <div className="out-of-stock">Out Of Stock</div>}
      <div className="item-icons">
        {!items.is_service && items.sell_over_stock == true && (
          <div className="inner-icon">
            <img src="/images/pos/card/over_sell.png" />
          </div>
        )}
        {items.type == 'package' && (
          <div className="inner-icon" onClick={() => handleShowPackageItems(items.product_id)}>
