      ...
      if (repo.status === true) {
        ...
        if (locationPermission(repo.data.locs, shopId) != -1) {
          if (subType == 'getProducts') {
            var con = doConnect();
            con.query(
              `SELECT products.id,products.type,products.name,products.image,products.sku,products.alert_quantity,products.sell_price,products.qty_over_sold,
            ...
            await con
              .promise()
              .query(
                `SELECT id as 'value',name as 'label' FROM tailoring_type WHERE location_id = ?`,
                [shopId]
              )
              .then((rows: any, fields: any) => {
                tailorings = rows[0];
            ...
            await con
              .promise()
              .query(
                `SELECT id as 'value',name as 'label',type,id as product_id,name,sku,sell_price AS price,cost_price as cost,qty_over_sold AS 'qty_sold'
                                FROM products 
            ...
            await con
              .promise()
              .query(
                `SELECT id as 'value',name as 'label' FROM brands WHERE brands.location_id = ?;`,
                [shopId]
              )
              .then((rows: any, fields: any) => {
                brands = rows[0];
            ...
            await con
              .promise()
              .query(
                `SELECT id as 'value',name as 'label' FROM categories WHERE categories.location_id = ? ORDER BY id desc`,
                [shopId]
              )
              .then((rows: any, fields: any) => {
                categories = rows[0];
            ...
            await con
              .promise()
              .query(
                `SELECT products.*,tailoring_package.tailoring_type_id,tailoring_package.prices_json,tailoring_package.fabric_ids
                                    FROM products 
            ...
            if (pro.length > 0) {
              await con
                .promise()
                .query(
                  `select package_items.product_id,package_items.product_id as id,package_items.id AS pack_id,package_items.price,products.name,1 AS quantity,false as 'isNew'
                                FROM package_items 
              ...
              await con
                .promise()
                .query(
                  `SELECT * FROM product_variations WHERE location_id = ? AND parent_id = ? and is_active = 1`,
                  [shopId, id]
                )
                .then((rows: any, fields: any) => {
                  variations = rows[0];
              ...
              await con
                .promise()
                .query(
                  `SELECT id as product_id,id,name,false as 'isNew'
                                    FROM products
            ...
            await con
              .promise()
              .query(
                `SELECT s.id as 'value',s.name as 'label',s.type,s.id as product_id,s.name,s.sku,s.sell_price AS price,s.cost_price as cost,categories.name AS 'category'
                            FROM products s
